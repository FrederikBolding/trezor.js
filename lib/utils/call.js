    ...
    _createClass(CallHelper, [{
        ...
        value: function _filterCommonTypes(res) {
            ...
            if (res.type === 'Deprecated_PassphraseStateRequest') {
                if (this.session.device) {
                    ...
                    if (currentState != null && currentState !== receivedState) {
                        ...
                        return this._commonCall('Cancel', {}).catch(function () {
                            _this5.session.errorEvent.emit(new Error('Invalid passphrase'));
                        });
                    }
            ...
            if (res.type === 'PassphraseRequest') {
                ...
                return this._promptPassphrase().then(function (res) {
                    var passphrase = res.passphrase,
                        onDevice = res.onDevice;

                    var session_id = _this5.session.device && _this5.session.device.features.session_id;
                    var passphraseState = _this5.session.device && _this5.session.device.passphraseState;
                    if (session_id) {
                        return _this5._commonCall('PassphraseAck', onDevice ? { on_device: true } : { passphrase: passphrase });
                    } else if (passphraseState) {
                        return _this5._commonCall('PassphraseAck', {
            ...
            if (res.type === 'Features') {
                var oldSessionId = this.session.device && this.session.device.features.session_id;
        ...
        value: function _promptPassphrase() {
            ...
            return new Promise(function (resolve, reject) {
                if (!_this7.session.passphraseEvent.emit(function (err, passphrase, onDevice) {
                    ...
                    return resolve({ passphrase: passphrase, onDevice: onDevice });
                })) {
